...
describe('descriptor', function () {
    ...
    it('should be able to check whether a bit name exists', function () {
        var d = new Descriptor({r: 3, w: 5});
        expect(d.check('r')).to.be.ok();
        expect(d.check('w')).to.be.ok();
        expect(d.check()).to.not.be.ok();
        expect(d.check('x')).to.not.be.ok();

        expect(d.check(['r'])).to.be.ok();
        expect(d.check(['r', 'w'])).to.be.ok();
        expect(d.check([])).to.not.be.ok();
        expect(d.check(['x', 'y'])).to.not.be.ok();
        expect(d.check(['r', 'x'])).to.not.be.ok();
    });
